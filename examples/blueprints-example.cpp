...
static bool Splitter(bool split_vertically, float thickness, float* size1, float* size2, float min_size1, float min_size2,
                     float splitter_long_axis_size = -1.0f)
{
    ...
    bb.Max =
        bb.Min + CalcItemSize(split_vertically ? ImVec2(thickness, splitter_long_axis_size) : ImVec2(splitter_long_axis_size, thickness),
                              0.0f, 0.0f);
...
struct Editor
{
    ed::Config         config{};         // yikes this thing has a nonsense default constructor
    ed::EditorContext* editor{nullptr};  // Editor indirectly takes addresses out of config which gets copied *facepalm*
    int                next_id = 1;
    ...
    Editor()
    {
        ...
        config.LoadNodeSettings = [](ed::NodeId nodeId, char* data, void* userPointer) -> size_t
        {
            ...
            auto node = self->find_node(nodeId);
            if (!node) return 0;
        ...
        links.push_back(Link(get_next_link_id(), nodes[5].Outputs[0].ID, nodes[6].Inputs[0].ID));
        links.push_back(Link(get_next_link_id(), nodes[5].Outputs[0].ID, nodes[7].Inputs[0].ID));
    ...
    Node* spawn_input_action_node()
    {
        nodes.emplace_back(get_next_id(), "InputAction Fire", ImColor(255, 128, 128));
        nodes.back().Outputs.emplace_back(get_next_id(), "", PinType::Delegate);
        nodes.back().Outputs.emplace_back(get_next_id(), "Pressed", PinType::Flow);
        nodes.back().Outputs.emplace_back(get_next_id(), "Released", PinType::Flow);

        nodes.back().update_in_outs();
    ...
    Node* SpawnBranchNode()
    {
        nodes.emplace_back(get_next_id(), "Branch");
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Flow);
        nodes.back().Inputs.emplace_back(get_next_id(), "Condition", PinType::Bool);
        nodes.back().Outputs.emplace_back(get_next_id(), "True", PinType::Flow);
        nodes.back().Outputs.emplace_back(get_next_id(), "False", PinType::Flow);
        return &nodes.back();
    }

    Node* SpawnDoNNode()
    {
        nodes.emplace_back(get_next_id(), "Do N");
        nodes.back().Inputs.emplace_back(get_next_id(), "Enter", PinType::Flow);
        nodes.back().Inputs.emplace_back(get_next_id(), "N", PinType::Int);
        nodes.back().Inputs.emplace_back(get_next_id(), "Reset", PinType::Flow);
        nodes.back().Outputs.emplace_back(get_next_id(), "Exit", PinType::Flow);
        nodes.back().Outputs.emplace_back(get_next_id(), "Counter", PinType::Int);
        return &nodes.back();
    }

    Node* spawn_output_action_node()
    {
        nodes.emplace_back(get_next_id(), "OutputAction");
        nodes.back().Inputs.emplace_back(get_next_id(), "Sample", PinType::Float);
        nodes.back().Outputs.emplace_back(get_next_id(), "Condition", PinType::Bool);
        nodes.back().Inputs.emplace_back(get_next_id(), "Event", PinType::Delegate);
        return &nodes.back();
    }

    Node* SpawnPrintStringNode()
    {
        nodes.emplace_back(get_next_id(), "Print String");
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Flow);
        nodes.back().Inputs.emplace_back(get_next_id(), "In String", PinType::String);
        nodes.back().Outputs.emplace_back(get_next_id(), "", PinType::Flow);
        return &nodes.back();
    }

    Node* SpawnMessageNode()
    {
        nodes.emplace_back(get_next_id(), "", ImColor(128, 195, 248));
        nodes.back().Type = NodeType::Simple;
        nodes.back().Outputs.emplace_back(get_next_id(), "Message", PinType::String);
        return &nodes.back();
    };

    Node* SpawnSetTimerNode()
    {
        nodes.emplace_back(get_next_id(), "Set Timer", ImColor(128, 195, 248));
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Flow);
        nodes.back().Inputs.emplace_back(get_next_id(), "Object", PinType::Object);
        nodes.back().Inputs.emplace_back(get_next_id(), "Function Name", PinType::Function);
        nodes.back().Inputs.emplace_back(get_next_id(), "Time", PinType::Float);
        nodes.back().Inputs.emplace_back(get_next_id(), "Looping", PinType::Bool);
        nodes.back().Outputs.emplace_back(get_next_id(), "", PinType::Flow);
        return &nodes.back();
    }
    ...
    Node* SpawnLessNode()
    {
        nodes.emplace_back(get_next_id(), "<", ImColor(128, 195, 248));
        nodes.back().Type = NodeType::Simple;
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Float);
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Float);
        nodes.back().Outputs.emplace_back(get_next_id(), "", PinType::Float);
        return &nodes.back();
    }

    Node* SpawnWeirdNode()
    {
        nodes.emplace_back(get_next_id(), "o.O", ImColor(128, 195, 248));
        nodes.back().Type = NodeType::Simple;
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Float);
        nodes.back().Outputs.emplace_back(get_next_id(), "", PinType::Float);
        nodes.back().Outputs.emplace_back(get_next_id(), "", PinType::Float);
    ...
    {
        nodes.emplace_back(get_next_id(), "Single Line Trace by Channel", ImColor(255, 128, 64));
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Flow);
        nodes.back().Inputs.emplace_back(get_next_id(), "Start", PinType::Flow);
        nodes.back().Inputs.emplace_back(get_next_id(), "End", PinType::Int);
        nodes.back().Inputs.emplace_back(get_next_id(), "Trace Channel", PinType::Float);
        nodes.back().Inputs.emplace_back(get_next_id(), "Trace Complex", PinType::Bool);
        nodes.back().Inputs.emplace_back(get_next_id(), "Actors to Ignore", PinType::Int);
        nodes.back().Inputs.emplace_back(get_next_id(), "Draw Debug Type", PinType::Bool);
        nodes.back().Inputs.emplace_back(get_next_id(), "Ignore Self", PinType::Bool);
        nodes.back().Outputs.emplace_back(get_next_id(), "", PinType::Flow);
        nodes.back().Outputs.emplace_back(get_next_id(), "Out Hit", PinType::Float);
        nodes.back().Outputs.emplace_back(get_next_id(), "Return Value", PinType::Bool);
        return &nodes.back();
    }

    Node* SpawnTreeSequenceNode()
    {
        nodes.emplace_back(get_next_id(), "Sequence");
    ...
    Node* SpawnTreeTaskNode()
    {
        nodes.emplace_back(get_next_id(), "Move To");
        nodes.back().Type = NodeType::Tree;
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Flow);
        return &nodes.back();
    }
    ...
    Node* SpawnTreeTask2Node()
    {
        nodes.emplace_back(get_next_id(), "Random Wait");
        nodes.back().Type = NodeType::Tree;
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Flow);
        return &nodes.back();
    };
    ...
    Node* SpawnComment()
    {
        nodes.emplace_back(get_next_id(), "Test Comment");
    ...
    Node* SpawnHoudiniTransformNode()
    {
        nodes.emplace_back(get_next_id(), "Transform");
        nodes.back().Type = NodeType::Houdini;
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Flow);
        nodes.back().Outputs.emplace_back(get_next_id(), "", PinType::Flow);
        return &nodes.back();
    }
    ...
    Node* SpawnHoudiniGroupNode()
    {
        nodes.emplace_back(get_next_id(), "Group");
        nodes.back().Type = NodeType::Houdini;
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Flow);
        nodes.back().Inputs.emplace_back(get_next_id(), "", PinType::Flow);
        nodes.back().Outputs.emplace_back(get_next_id(), "", PinType::Flow);
    ...
    void build_nodes()
    {
        for (auto& node : nodes) node.update_in_outs();
    }

    int        get_next_id() { return next_id++; }
    ed::LinkId get_next_link_id() { return ed::LinkId(get_next_id()); }
    void       touch_node(ed::NodeId id) { node_touch_time[id] = 1.0f; }
    float      get_touch_progress(ed::NodeId id)
    {
        auto it = node_touch_time.find(id);
        return (it != node_touch_time.end() && it->second > 0.0f) ? (1.0f - it->second) / 1.0f : 0.0f;
    }

    void update_touch()
    {
        const auto deltaTime = ImGui::GetIO().DeltaTime;
        for (auto& entry : node_touch_time)
            if (entry.second > 0.0f) entry.second -= deltaTime;
    }

    auto find_node(ed::NodeId id) -> Node*
    {
        for (auto& node : nodes)
    ...
    auto find_link(ed::LinkId id) -> Link*
    {
        for (auto& link : links)
    ...
    auto is_pin_linked(ed::PinId id) -> bool
    {
        if (!id) return false;
    ...
    auto can_create_link(Pin* a, Pin* b) -> bool
    {
        return !(!a || !b || a == b || a->Kind == b->Kind || a->Type != b->Type || a->Node == b->Node);
    }
    ...
    void operator()(imgui::Context* ctx)
    {
        ...
        ed::Begin("Node editor");
        {
            ...
            util::BlueprintNodeBuilder builder(get_header_background(), 800 /*GetTextureWidth(get_header_background())*/,
                                               400 /*GetTextureHeight(get_header_background())*/);
            ...
            for (auto& node : nodes)
            {
                ...
                if (!isSimple)
                {
                    ...
                    if (hasOutputDelegates)
                    {
                        ...
                        for (auto& output : node.Outputs)
                        {
                            ...
                            if (newLinkPin && !can_create_link(newLinkPin, &output) && &output != newLinkPin)
                            ...
                            DrawPinIcon(output, is_pin_linked(output.ID), (int)(alpha * 255));
                ...
                {
                    auto alpha = ImGui::GetStyle().Alpha;
                    if (newLinkPin && !can_create_link(newLinkPin, &input) && &input != newLinkPin) alpha = alpha * (48.0f / 255.0f);

                    builder.Input(input.ID);
                    ImGui::PushStyleVar(ImGuiStyleVar_Alpha, alpha);
                    DrawPinIcon(input, is_pin_linked(input.ID), (int)(alpha * 255));
                    ImGui::Spring(0);
                    if (!input.Name.empty())
                    {
                ...
                for (auto& output : node.Outputs)
                {
                    ...
                    auto alpha = ImGui::GetStyle().Alpha;
                    if (newLinkPin && !can_create_link(newLinkPin, &output) && &output != newLinkPin) alpha = alpha * (48.0f / 255.0f);
            ...
            for (auto& node : nodes)
            {
                ...
                if (!node.Inputs.empty())
                {
                    ...
                    if (newLinkPin && !can_create_link(newLinkPin, &pin) && &pin != newLinkPin)
                ...
                // const auto ImDrawList_AddRect = [](ImDrawList* drawList, const ImVec2& a, const ImVec2& b, ImU32 col, float
                // rounding, int rounding_corners, float thickness)
                //{
                //     if ((col >> 24) == 0)
                //         return;
            ...
            for (auto& node : nodes)
            {
                ...
                //     IM_COL32((int)(255 * pinBackground.x), (int)(255 * pinBackground.y), (int)(255 * pinBackground.z),
                //     inputAlpha), 4.0f, 12);
                // ImGui::PushStyleVar(ImGuiStyleVar_AntiAliasFringeScale, 1.0f);
            ...
            for (auto& node : nodes)
            {
                ...
                if (ed::BeginGroupHint(node.ID))
                {
                    ...
                    drawList->AddRectFilled(hintFrameBounds.GetTL(), hintFrameBounds.GetBR(), IM_COL32(255, 255, 255, 64 * bgAlpha / 255),
                                            4.0f);
            ...
            for (auto& link : links) ed::Link(link.ID, link.StartPinID, link.EndPinID, link.Color, 2.0f);
                    ...
                    if (ed::QueryNewLink(&startPinId, &endPinId))
                    {
                        ...
                        if (startPin && endPin)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    links.emplace_back(Link(get_next_id(), startPinId, endPinId));
        ...
        {
            auto node = find_node(contextNodeId);
        ...
        {
            auto pin = find_pin(contextPinId);
        ...
        {
            auto link = find_link(contextLinkId);
        ...
        if (ImGui::BeginPopup("Create New Node"))
        {
            ...
            Node* node = nullptr;
            if (ImGui::MenuItem("Input Action")) node = spawn_input_action_node();
            if (ImGui::MenuItem("Output Action")) node = spawn_output_action_node();
            ...
            if (node)
            {
                ...
                if (auto startPin = newNodeLinkPin)
                {
                    ...
                    for (auto& pin : pins)
                    {
                        if (can_create_link(startPin, &pin))
                        {
                            auto endPin = &pin;
                            if (startPin->Kind == PinKind::Input) std::swap(startPin, endPin);

                            links.emplace_back(Link(get_next_id(), startPin->ID, endPin->ID));
                            links.back().Color = GetIconColor(startPin->Type);
        ...
        /*
            ...
            drawList->AddBezierCurve(to_imvec(c.p0), to_imvec(c.p1), to_imvec(c.p2), to_imvec(c.p3), IM_COL32(255, 255, 255,
           255), 1.0f); cubic_bezier_subdivide(acceptPoint, c);
        */
    ...
    void ShowStyleEditor(bool* show = nullptr)
    {
        ...
        ImGui::Spring();
        if (ImGui::Button("Reset to defaults")) editorStyle = ed::Style();
        ImGui::EndHorizontal();
        ImGui::Spacing();
    ...
    void ShowLeftPane(float paneWidth)
    {
        ...
        {
            for (auto& link : links) ed::Flow(link.ID);
        }
        ImGui::Spring();
        ...
        for (auto& node : nodes)
        {
            ...
            if (const auto progress = get_touch_progress(node.ID))
            {
            ...
            if (node.SavedState.empty())
            {
                ...
                if (ImGui::IsItemActive())
                    drawList->AddImage(get_save_icon(), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1),
            ...
            if (!node.SavedState.empty())
            {
                ...
                if (ImGui::IsItemActive())
                    drawList->AddImage(get_restore_icon(), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1),
                                       IM_COL32(255, 255, 255, 96));
                else if (ImGui::IsItemHovered())
                    drawList->AddImage(get_restore_icon(), ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), ImVec2(0, 0), ImVec2(1, 1),
                                       IM_COL32(255, 255, 255, 255));
...
int                             main()
{
    ...
    ui_context =
        imgui::create_ui([](imgui::Context* ctx) { (*editor)(ctx); }, std::make_unique<imgui::DefaultStyle>(), "ImGui Example", 1200, 800);
    ui_context->run();
}
