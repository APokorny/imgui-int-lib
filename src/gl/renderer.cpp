...
ig::Renderer::Renderer(int version_num, std::string const& version) : glsl_version{version_num}, glsl_version_text{version} {}
...
void ig::Renderer::render_imgui_data(ImDrawData& draw_data)
{
    ...
    auto restore_poly =
        restore<GLint[2]>(GL_POLYGON_MODE, [](GLint lm[2]) { glPolygonMode(GL_FRONT_AND_BACK, static_cast<GLenum>(lm[0])); });
#endif
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                {
                    ...
                    // Bind texture, Draw
                    glBindTexture(GL_TEXTURE_2D, static_cast<GLuint>(reinterpret_cast<ptrdiff_t>(pcmd->TextureId)));
