...
int                             main()
{
    ui_context = imgui::create_ui(
        [show_demo_window = true, show_another_window = false, clear_color = ImVec4(0.45f, 0.55f, 0.60f, 1.00f)](imgui::Context*) mutable {
            ...
            // Begin/End pair to created a named window.
            {
                ...
                static auto tp = std::chrono::system_clock::now();
                imgui::day_time_picker("foo", tp);
                static int level = 0;
                using years      = std::chrono::duration<int32_t, std::ratio<31556952>>;
                imgui::date_picker("foo", level, tp, tp - years{20}, tp + years{1});
